...
paramss = sorted(glob.glob('param*.py'))
...
print 'param sets', len(paramss), 'total runs', len(paramscomb) * len(paramss)
# model definitions
argv = int(sys.argv[1])
nparam, njob = divmod(argv-1, len(paramscomb))
params = paramss[nparam]
p = __import__(params[:-3])
lambda_ = p.lambda_
mus = p.mus
cup = p.cup
...
if parametercheck(datadir, sys.argv, paramscomb, nbatch):
    ...
    np.savez_compressed(datadir + 'scan_phases_%s_%g' % (params[-4], njob+1), data=data, columns=columns)
